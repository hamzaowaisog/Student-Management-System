...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    while($row = mysqli_fetch_assoc($result)){
        ...
        while($row2 = mysqli_fetch_assoc($result2)){
                ...
                $grade_data .= "<td>$total_session</td>";
                $grade_data .= "<td>$session_attend</td>";
    ...
    $pagination = '';
    if ($page > 1) {
        $pagination .= '<a href="#" onclick="loadattendance(' . $teacher_id . ', ' . $course_id . ', ' . ($page - 1) . ')">Previous</a>';
    }
    for ($i = 1; $i <= $total_pages; $i++) {
        $pagination .= '<a href="#" onclick="loadattendance(' . $teacher_id . ', ' . $course_id . ', ' . $i . ')"';
        if ($i == $page) $pagination .= 'class="active"';
        $pagination .= '>' . $i . '</a>';
    }
    if ($page < $total_pages) {
        $pagination .= '<a href="#" onclick="loadattendance(' . $teacher_id . ', ' . $course_id . ', ' . ($page + 1) . ')">Next</a>';
    }
